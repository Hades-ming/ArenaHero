...
## `PERF-002` 空载 Worker A* 预算减半（当前 LIVE_CANARY）
...
- **21-Tick canary**：`t66036..t66056` 无失败、缺口、Core/Unit 损失、资源失败或 Core 移动失败，
  只有 `1` 次 `CELL_UNIT_LIMIT`；入核 `3`、完整链路 `1`（延迟 `11` Tick），但其中存在重启边界携货，
  不用于宣称收益提升。
- **性能结果**：A* 共调用 `149` 次、展开 `254607`、预算耗尽 `64` 次、成功路径 `85` 条；单 Tick
  展开 P50/P95 `12502/20576`，较 PERF-001 的 `22649.5/37918.8` 分别下降约 `44.8%/45.7%`。
  决策 P50/P95 `204/385ms`，较前窗 `268/615.25ms` 下降约 `23.9%/37.4%`；当前 canary 护栏通过，
  继续累计到 `100` Tick/`20` 条完整链路后再决定接受或回滚。
